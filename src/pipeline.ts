...
function runFlatten(inputPath: string, runDir: string, raw: RawTrajectory[]) {
...
function runIr(raw: RawTrajectory[], runDir: string): { irPath: string; trajectories: TrajectoryIR[] } {
  banner("Stage 1/5: IR Normalization");
  const data = cursorIr(raw);
...
function runReport(
  ...
  for (const traj of trajectories) {
    const report = buildReport(traj, crMap.get(traj.trajectory_id)!, attMap.get(traj.trajectory_id)!);
  ...
  console.log(`  Wrote reports/ and appended attribution to ${flatMdPath}`);
}
...
export function processFile(inputPath: string, runName?: string, skipJudge = false) {
  ...
  const flatMdPath = runFlatten(inputPath, runDir, raw);
  const { trajectories } = runIr(raw, runDir);
