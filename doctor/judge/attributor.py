...
def attribute(checker_result: dict[str, Any], traj: dict[str, Any]) -> dict[str, Any]:
    ...
    # critical step: earliest high/critical in primary category
    ...
    confidence = round(category_scores[primary] / max(total_score, 1), 2)

    top_violations = sorted(violations, key=lambda v: -SEVERITY_WEIGHT.get(v.get("severity", "low"), 1))[:5]

    explanation_parts = [f"Primary cause: {primary} (score {category_scores[primary]:.0f}/{total_score:.0f})"]
    ...
    return {
        ...
        "recommended_actions": ACTIONS.get(primary, []),
